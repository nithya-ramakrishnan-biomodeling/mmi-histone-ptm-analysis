...
class Histone_Regressor:
    ...
    def scatterplotter(
        ...
        data_range = max(predicted_output[0]) - min(predicted_output[0])
        step = max(1, int(data_range / 8))  # Limit to ~8 ticks
        ticks_values = np.arange(
            min(predicted_output[0]), max(predicted_output[0]), step, dtype=int
        )
        ...
        if file_name:
            output_dir_rel_path = os.path.join("..", "..", "output", "images")
            output_dir = os.path.abspath(output_dir_rel_path)
