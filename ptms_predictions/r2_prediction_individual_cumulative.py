...
parser = argparse.ArgumentParser(description="Calculate individual and cumulative R² scores for histone modifications")
...
def calculate_correlations(histone_mod_df: pd.DataFrame, target_histone: str, feature_histones: list):
    """
...
def calculate_augmented_r2(histone_mod_df: pd.DataFrame, target_histone: str, base_features: list, extra_feature: str):
    """
...
def calculate_cumulative_r2(histone_mod_df: pd.DataFrame, target_histone: str, base_features: list, added_features: list):
    """
...
def run_augmented_r2_parallel(histone_mod_df: pd.DataFrame, target_histone: str, base_features: list, extra_features: list, n_jobs: int = 2):
    ...
    try:
        ...
        results = Parallel(n_jobs=n_jobs, max_nbytes='50M')(
            delayed(calculate_augmented_r2)(histone_mod_df, target_histone, base_features, feature)
        ...
        for feature in tqdm(extra_features, desc="Calculating augmented R²"):
            result = calculate_augmented_r2(histone_mod_df, target_histone, base_features, feature)
...
if __name__ == "__main__":
    ...
    if clean_start:
        individual_output = os.path.join(output_dir_full_path, f"{organism}_{target_histone}_individual_r2.json")
        cumulative_output = os.path.join(output_dir_full_path, f"{organism}_{target_histone}_cumulative_r2.json")
        r2_values_output = os.path.join(output_dir_full_path, f"{organism}_{target_histone}_r2_values.json")
    ...
    try:
        ...
        print(f"Found {len(base_feature_list)} base feature histones and {len(target_list)} target histones")
        
        # Verify our target histone is in the target list
        if target_histone not in target_list:
            print(f"Warning: Target histone '{target_histone}' is not in the target list from feature/target split")
            print("This means your target is likely a 'feature' in the original methodology")
    except Exception as e:
    ...
    if base_feature_list:
        missing_features = [f for f in base_feature_list if f not in histone_mod_df.columns]
        if missing_features:
            print(f"Warning: {len(missing_features)} base features not found in dataset: {', '.join(missing_features)}")
    ...
    available_target_histones = [h for h in target_list if h in histone_mod_df.columns and h != target_histone]
    print(f"Found {len(available_target_histones)} available target histones (excluding the target itself)")
    
    # Calculate correlations with the target (for all target histones)
    print(f"Calculating correlations with {target_histone}...")
    correlations = calculate_correlations(histone_mod_df, target_histone, available_target_histones)
    ...
    print(f"Individual R² calculation complete: {valid_results}/{len(sorted_target_histones)} valid results")
    ...
    added_features = []
    for i in tqdm(range(1, len(sorted_target_histones) + 1), desc="Calculating cumulative R²"):
        ...
        _, r2 = calculate_cumulative_r2(histone_mod_df, target_histone, base_features, added_features)
    ...
    json_file_saver(save_dict=cumulative_r2_dict, absolute_file_name=cumulative_output_path)
    ...
    json_file_saver(save_dict=simplified_r2_dict, absolute_file_name=r2_values_output_path)
