...
parser = argparse.ArgumentParser(
    description="R² prediction using pseudo-inverse based features"
)
...
pseudo_inverse_file = os.path.join(
    ...
    "pseudo_inverse",
    organism,
    f"{organism}_pseudo_inverse.json",
)
...
def regression_prediction(target_histone: str):
    ...
    if target_histone not in pseudo_inverse_features:
        print(
            f"Warning: {target_histone} not found in pseudo_inverse_features. Skipping..."
        )
        return None
    ...
    if missing_features:
        print(
            f"Warning: Features {missing_features} for {target_histone} not found in data. Skipping..."
        )
        return None
...
if parallel:
    ...
    results = {
        h: r
        for h, r in zip(pseudo_inverse_features.keys(), results_list)
        if r is not None
    }
else:
    for target_histone in pseudo_inverse_features.keys():
...
output_file = os.path.join(
    output_dir_full_path, f"{organism}_pseudo_inverse_xgb_r2.json"
)
