...
PLOT_STYLE = {"fontsize": 12, "family": "serif", "weight": "bold"}
...
class HistoneRegressor:
                    ...
                    (
                        ...
                        SelectFromModel(
                            OrthogonalMatchingPursuitCV(
                                cv=omp_cv, max_iter=n_features_to_select
                            ),
    ...
    def select_top_n_features(self, n_features: int) -> np.ndarray:
        ...
        omp = OrthogonalMatchingPursuitCV(
            cv=self.omp_cv, max_iter=min(X_scaled.shape[1], 30)
        )
        ...
        if self.feature_names is not None:
            self.selected_feature_names = [
                self.feature_names[i] for i in top_n_features
            ]
        ...
        if self.feature_names is not None:
            if len(self.selected_features) > 0:
                ...
                if len(self.feature_names) >= max(self.selected_features) + 1:
                    ...
                    self.selected_feature_names = [
                        f"feature_{i+1}" for i in self.selected_features
                    ]
            else:
                self.selected_feature_names = []
    ...
    def _calculate_error_with_cv(self, method: Literal["r2", "mse", "rmse"]) -> float:
        ...
        if self.n_features_to_select is not None:
            ...
            for train_idx, test_idx in kfold.split(self.features):
                ...
                omp = OrthogonalMatchingPursuitCV(
                    cv=5, max_iter=min(X_train_scaled.shape[1], 30)
                )
    ...
    def plot_feature_importance(
        ...
        if (
            not hasattr(self, "selected_features")
            or self.selected_features is None
            or not self.selected_features.size
        ):
        ...
        if plot_type == "both":
            ...
            adjusted_height = max(
                12, n_features * 0.6
            )  # Minimum height of 12, but scales with feature count
        ...
        if plot_type in ["omp", "both"]:
            ...
            if self.selected_feature_names is not None:
                sorted_names = [
                    truncate_name(self.selected_feature_names[i], max_name_length)
            ...
            ax_omp.set_xticklabels(
                sorted_names, rotation=60, ha="right", fontsize=label_fontsize
            )
        ...
        if plot_type in ["xgboost", "both"]:
            ...
            if self.n_features_to_select is not None:
                ...
                if not hasattr(xgb_model, "feature_importances_"):
                    print(
                        "XGBoost model not trained yet for feature importance analysis."
                    )
                    return
            else:
            ...
            if hasattr(xgb_model, "feature_importances_"):
                ...
                if len(importances) > 0:
                    ...
                    if self.selected_feature_names is not None and len(
                        self.selected_feature_names
                    ) == len(importances):
                        # Properly map XGBoost importance indices to OMP-selected feature names
                        feature_labels = [
                            truncate_name(
                                self.selected_feature_names[i], max_name_length
                            )
                            for i in sorted_idx
                        ]
                    else:
                    ...
                    if plot_type == "both":
                        ax_xgb.set_title(
                            "XGBoost Feature Importance Not Available",
                            fontdict=PLOT_STYLE,
                        )
                    else:
                ...
                if plot_type == "both":
                    ax_xgb.set_title(
                        "XGBoost Feature Importance Not Available", fontdict=PLOT_STYLE
                    )
                else:
                    print("XGBoost feature importance not available.")
    ...
    def get_feature_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "n_selected_features": (
                len(self.selected_features) if self.selected_features is not None else 0
            ),
            "selected_indices": (
                self.selected_features.tolist()
        ...
        if (
            self.omp_coefficients is not None
            and self.selected_feature_names is not None
        ):
            coef_dict = {}
    ...
    def print_feature_summary(self) -> None:
        ...
        print(
            f"Selected features: {len(self.selected_features) if self.selected_features is not None else 0}"
        )

        if self.selected_features is not None and len(self.selected_features) > 0:
            print(
                f"Selection ratio: {len(self.selected_features)/len(self.feature_names):.2%}"
            )
            ...
            if self.n_features_to_select is None:
                xgb_model = self.model.named_steps["regressor"]
                if (
                    hasattr(xgb_model, "feature_importances_")
                    and len(xgb_model.feature_importances_) > 0
                ):
...
if __name__ == "__main__":
    ...
    model = HistoneRegressor(
        features=X, target=y, feature_names=feature_names, n_features_to_select=5
    )
