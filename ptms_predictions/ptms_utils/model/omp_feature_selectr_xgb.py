...
class ManualOMPHistoneRegressor:
    ...
    def _select_features_with_omp(
        ...
        if self.n_features_to_select is not None:
            ...
            selected_indices = np.argsort(coef_magnitudes)[::-1][
        ...
        if (
            ...
            and len(self.feature_names) >= max(selected_indices) + 1
        ):
            self.selected_feature_names = [
                self.feature_names[i] for i in selected_indices
            ]
        else:
    ...
    def train_and_predict(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        x_train_selected, selected_indices = self._select_features_with_omp(
    ...
    def plot_feature_importance(
        ...
        if plot_type in ["omp", "both"]:
            ...
            # Plot bars
            bars = ax_omp.bar(
                range(len(sorted_names)), np.abs(sorted_coefs), color=colors
            )
        ...
        if plot_type in ["xgboost", "both"]:
            ...
            if hasattr(self.xgb_model, "feature_importances_"):
                ...
                if len(importances) > 0:
                    ...
                    # Plot bars
                    ax_xgb.barh(
                        pos, importances[sorted_idx], align="center", color="#3F7C85"
                    )
    ...
    def print_feature_summary(self) -> None:
        ...
        if self.n_features_to_select is not None:
            print(
                f"Selection criteria: Top {self.n_features_to_select} features by coefficient magnitude"
            )
        else:
            print(
                f"Selection criteria: Coefficient magnitude > {self.min_coef_value:.2e}"
            )
        ...
        if (
            hasattr(self.xgb_model, "feature_importances_")
            and len(self.xgb_model.feature_importances_) > 0
        ):
...
if __name__ == "__main__":
    ...
    model.plot_feature_importance(
        plot_type="both", filename="threshold_based_importance"
    )
    model_fixed.plot_feature_importance(
        plot_type="both", filename="fixed_feature_importance"
    )
