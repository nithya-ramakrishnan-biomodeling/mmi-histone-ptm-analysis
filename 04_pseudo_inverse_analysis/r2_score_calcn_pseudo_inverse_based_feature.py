...
import argparse
import time
from multivariate_utils import ProjectPaths, dir_maker, load_data
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Pseudo-inverse feature selection and R² calculation"
    )
    parser.add_argument(
        "-o",
        "--organism",
    ...
    if parallel_mode:
        ...
        results = Parallel(n_jobs=-1)(
            delayed(process_target)(target, df) for target in df.columns
        )

        # Process results
        for target, features, r_squared in results:
            if features is not None and r_squared is not None:
                full_results[target] = {"features": features, "r_squared": r_squared}
