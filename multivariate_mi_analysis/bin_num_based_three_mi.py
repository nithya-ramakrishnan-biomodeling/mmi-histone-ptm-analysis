...
def process_organism_covering_triplets_mi(
    organism, use_global_minmax, bin_num_list, random_seed=None
):
    """
    Process a single organism, calculating three-variable mutual information for covering feature triplets.
    Uses diff_uy_and_uy_givenv from entropy_mutualinfo.
    """
    if random_seed is not None:
        np.random.seed(random_seed)
    ...
    for triplet_idx, (feature1, feature2, feature3) in enumerate(
        tqdm(histone_triplets, desc=f"Processing {organism} triplets (MI)"), 1
    ):
        ...
        min_val = global_min
        max_val = global_max
        # Uncomment below to use individual min/max
        # if not use_global_minmax:
        ...
        for bin_num in bin_num_list:
            bin_edges = np.linspace(min_val, max_val, bin_num)
            mi_val = entropy_mutualinfo.diff_uy_and_uy_givenv(
                feature1_values, feature2_values, feature3_values, bin_edges
            )
            mi_values[triplet_name]["mi_values"][bin_num] = mi_val
    ...
    output_dir_name = "covering_triplets_mi_values"
    ...
    output_file = os.path.join(
        output_dir, f"covering_triplets_mi_{minmax_type}_minmax.json"
    )
    ...
    print("\nThree-Variable Mutual Information Summary for Selected Bin Numbers:")
    print("Triplet".ljust(60), end="")
    for bin_num in summary_bins:
        print(f"{bin_num:>10}", end="")
    print()

    for triplet_name in list(mi_values.keys())[:10]:
        feature_triplet = f"{mi_values[triplet_name]['feature1']} - {mi_values[triplet_name]['feature2']} - {mi_values[triplet_name]['feature3']}"
        print(feature_triplet[:60].ljust(60), end="")
        for bin_num in summary_bins:
            if bin_num in mi_values[triplet_name]["mi_values"]:
                print(f"{mi_values[triplet_name]['mi_values'][bin_num]:>10.3f}", end="")
            else:
                print(f"{'N/A':>10}", end="")
        print()

    if len(mi_values) > 10:
        print("... plus", len(mi_values) - 10, "more triplets")

    # Also save a CSV summary for easier analysis
    summary_data = []
    for triplet_name, triplet_data in mi_values.items():
        row = {
        ...
        for bin_num in summary_bins:
            row[f"bin_{bin_num}"] = triplet_data["mi_values"].get(bin_num, np.nan)
        summary_data.append(row)

    summary_df = pd.DataFrame(summary_data)
    summary_file = os.path.join(
        output_dir, f"covering_triplets_mi_summary_{minmax_type}_minmax.csv"
    )
...
def main():
    ...
    for i, organism in enumerate(organisms, 1):
        ...
        all_results[organism] = process_organism_covering_triplets_mi(
    ...
    if len(organisms) > 1:
        minmax_type = "global" if use_global_minmax else "individual"
        output_dir = os.path.join(
            ProjectPaths.get_output_files_dir(), "covering_triplets_mi_values"
        )
        os.makedirs(output_dir, exist_ok=True)
        consolidated_file = os.path.join(
            output_dir,
            f"consolidated_covering_triplets_joint_mi_{minmax_type}_minmax.json",
        )
