...
class HeatmapColorPalette:
    ...
    DIVERGING = {
        'rwb': ['#67001F', '#B2182B', '#F4A582', '#FDDBC7', '#D1E5F0', '#92C5DE', '#2166AC'],
        'coolwarm': ['#4A148C', '#7B1FA2', '#E1BEE7', '#F5F5F5', '#B3E5FC', '#0288D1', '#01579B'],
    ...
    CORRELATION = {
        ...
        'pearson': ['#052F60', '#0C5DC4', '#89C4FF', '#FFFFFF', '#FF9B9B', '#D92121', '#7A0000']
    }
    
    @staticmethod
    def get_heatmap_colors(n_colors: int, 
        ...
        if palette_type == 'sequential':
            base_colors = HeatmapColorPalette.SEQUENTIAL.get(
                scheme or 'viridis'
            )
        elif palette_type == 'diverging':
            base_colors = HeatmapColorPalette.DIVERGING.get(
                scheme or 'rwb'
            )
        else:
            base_colors = HeatmapColorPalette.CORRELATION.get(
                scheme or 'correlation'
            )
