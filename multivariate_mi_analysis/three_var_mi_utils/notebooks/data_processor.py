...
def masked_df_gen(
    ...
    * Eg. Getting the dataframe values greater than 5:
    ...
    if len(threshold_value) == 1 and len(oprtr) == 1:
        ...
        print(
            f"This is what I got from the input operator: {oprtr} and Threshold:{threshold_value} values."
        )


def two_mi_value_combn_three_mi_value_gen(
    three_mi_df: pd.DataFrame,
    ...
    for one, two, three in unique_three_var_combn:
        ...
        two_var_combn = np.array(
            [two_mi_df[col][indx] for col, indx in combinations([one, two, three], r=2)]
        )
        ...
        if len(cond_value) == 2:
            ...
            if cond_comparisonr(float(three_mi), lt_oprtor, lt) and cond_comparisonr(
                float(three_mi), right_oprtr, gt
            ):
                two_mi_three_var_value[f"{str(3)}_mi"] = three_mi
                two_mi_three_var_value[f"{str(2)}_mi"] = {
                    (col, indx): two_mi_df[col][indx]
                    for col, indx in combinations([one, two, three], r=2)
                }

                if isinstance(cov_df, pd.DataFrame):
                    two_mi_three_var_value["cov"] = {
                        (col, indx): cov_df[col][indx]
                        for col, indx in combinations([one, two, three], r=2)
                    }

                if isinstance(corr_df, pd.DataFrame):
                    two_mi_three_var_value["corr"] = {
                        (col, indx): corr_df[col][indx]
                        for col, indx in combinations([one, two, three], r=2)
                    }
            ...
            if no_elemnts > 0 and cond_comparisonr(
                float(three_mi), cond_value[0], oprtr[0]
            ):
                two_mi_three_var_value[f"{str(3)}_mi"] = three_mi
                two_mi_three_var_value[f"{str(2)}_mi"] = {
                    (col, indx): two_mi_df[col][indx]
...
def df_and_text_generator(input_dict: dict, df_new_indx: list, df_new_col: list):
    ...
    three_mi_neg_two_mi_pos_df_hover = pd.DataFrame(
        index=df_new_indx, columns=df_new_col
    )
    ...
    for i in input_dict.items():
        ...
        cov_dict = {
            "Cov": {str(key): round(value, 3) for key, value in i[1]["cov"].items()}
        }
        corr_dict = {
            "Corr": {str(key): round(value, 3) for key, value in i[1]["corr"].items()}
        }
        # Doing json dump for transforming the dictionary into string format.
        converted_dict = {
            f"{str(2)}_mi": {
                str(key): round(value, 3) for key, value in orginal_dict.items()
            }
        }

        combind_dict = (
            converted_dict | cov_dict | corr_dict
        )  # Combining the dictioanry.
        ...
        three_mi_neg_two_mi_pos_df_hover.loc[indx, col] = [
            clean_json(
                json.dumps(combind_dict, indent=2), unwanted_chars=unwanted_chars
            ),
        ]

    three_mi_neg_two_mi_pos_df_row = three_mi_neg_two_mi_pos_df.dropna(
        axis=0, how="all"
    )  # Dropping the rows, if all are empty.
    three_mi_neg_two_mi_pos_df_col = three_mi_neg_two_mi_pos_df_row.dropna(
        axis=1, how="all"
    )  # Dropping colums, if all are empty.
    three_mi_neg_two_mi_pos_df_row_hover = three_mi_neg_two_mi_pos_df_hover.dropna(
        axis=0, how="all"
    )  # Dropping the rows, if all are empty.
    three_mi_neg_two_mi_pos_df_col_hover = three_mi_neg_two_mi_pos_df_row_hover.dropna(
        axis=1, how="all"
    )  # Dropping the rows, if all are empty.
