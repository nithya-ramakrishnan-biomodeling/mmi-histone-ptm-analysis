...
def process_organism(organism, use_global_minmax, bin_num_list):
    ...
    for feature in tqdm(data.columns, desc=f"Processing {organism} features"):
        ...
        for bin_num in bin_num_list:
            ...
            entropy = entropy_mutualinfo.mi_betwn_uandy(
                feature_values, feature_values, bin_edges
            )
    ...
    print(
        f"Processed {len(data.columns)} features in {int(minutes)} minutes and {seconds:.2f} seconds"
    )
    ...
    output_file = os.path.join(
        output_dir, f"all_ptms_bin_based_entropy_values_{minmax_type}_minmax.json"
    )
    ...
    for bin_num in summary_bins:
        summary_df[f"bin_{bin_num}"] = [
            entropy_values[feature][bin_num] for feature in data.columns
        ]

    summary_file = os.path.join(
        output_dir, f"bin_entropy_summary_{minmax_type}_minmax.csv"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate entropy values for histone PTMs"
    )
    parser.add_argument(
        "-o",
        "--organisms",
    ...
    for i, organism in enumerate(organisms, 1):
        ...
        all_results[organism] = process_organism(
    ...
    if len(organisms) > 1:
        ...
        consolidated_file = os.path.join(
            output_dir, f"consolidated_entropy_values_{minmax_type}_minmax.json"
        )
