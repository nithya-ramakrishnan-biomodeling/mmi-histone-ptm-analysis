...
def run_mmi_calculations(organism, project_dir, parallel, cpu_limit):
    ...
    for operator, cutoff_list in organism_cutoffs.items():
        print(f"    Processing operator '{operator}' with cutoffs: {', '.join(cutoff_list)}")
...
def run_remaining_features_calculation(organism, project_dir, parallel, cpu_limit):
    ...
    result = run_command(
        remain_cmd, 
        f"All-remaining features: {organism}"
    )
...
def main():
    ...
    for organism in organisms:
        ...
        print(f"  {organism}: {organism_total} tasks ({mmi_tasks} MMI + {remaining_tasks} remaining)")
    ...
    print(f"Total runtime: {int(hours)} hours, {int(minutes)} minutes and {seconds:.2f} seconds")
    ...
    for organism in organisms:
        ...
        print(f"    MMI tasks: {mmi_tasks} (cutoffs: {', '.join([', '.join(cutoffs) for cutoffs in organism_cutoffs.values()])})")
