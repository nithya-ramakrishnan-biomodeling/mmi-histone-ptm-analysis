...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate mutual information using MMI-selected features"
    )
    parser.add_argument(
        "-o",
        "--organism",
    ...
    parser.add_argument(
        "-c", "--cutoff", type=float, required=True, help="MMI cutoff value"
    )
    parser.add_argument(
        "-op",
        "--operator",
...
def main():
    ...
    data_file_path = os.path.join(
        ProjectPaths.get_cleaned_data_dir(), organism, data_file_name
    )

    # Create output directory
    output_dir = os.path.join(
        ProjectPaths.get_output_files_dir(), "mi_values_from_mmi_cutoff", organism
    )
    ...
    feature_target_json_file_dir = os.path.join(
        ...
        organism,
        "mmi_r2",
        f"{organism}_mmi_cutoff_{cutoff}_{operator}_feature_target.json",
    )  # Include operator

    # Output file path should also include the operator
    mi_values_file = os.path.join(
        output_dir, f"mi_values_{str(cutoff)}_{operator}.json"
    )
    ...
    if parallel_mode:
        ...
        try:
            ...
            for target_name in tqdm(targets, desc="Processing targets"):
                target_name, mi_value = calculate_mi_for_target(
                    target_name, feature_df, df, bin_edges
                )
                mi_values[target_name] = mi_value
    else:
