...
parser = argparse.ArgumentParser(
    description="Calculate mutual information using all remaining features approach"
)
...
def calculate_mutual_info(target_histone: str, histone_mod_df: pd.DataFrame):
    ...
    try:
        ...
        ml_model = Histone_Regressor(
            feature=feature_histone_mod_df, target=target_values
        )
        ...
        mi_value = entropy_mutualinfo.mi_betwn_uandy(
            arr1=y_test, arr2=y_pred, spaced_bin_edges=bin_edges
        )

        # Calculate MI between y_test and itself (self-information)
        self_info_value = entropy_mutualinfo.mi_betwn_uandy(
            arr1=y_test, arr2=y_test, spaced_bin_edges=bin_edges
        )
...
if __name__ == "__main__":
    print(
        f"Starting MI calculation with {'sequential mode' if sequential_mode else f'{cpu_limit} CPU cores'}"
    )
                ...
                try:
                    ...
                    for key, val in temp_dict.items():
                        if isinstance(val, str) and val.startswith("Error:"):
                            print(
                                f"Warning: Previous result for {key} contains an error. Will recalculate."
                            )
                            valid_results = False
                            break
                    ...
                    if valid_results:
                        ...
                        print(
                            "Previous results contain errors, starting from an earlier batch"
                        )
                except Exception as e:
    ...
    for batch_idx, batch in enumerate(batches[latest_batch:], start=latest_batch):
        ...
        if check_memory_usage() > memory_limit:
            print(
                f"WARNING: Memory usage ({check_memory_usage():.2f} MB) exceeds limit ({memory_limit} MB)"
            )
        ...
        if len(batch_to_process) < len(batch):
            print(
                f"Skipping {len(batch) - len(batch_to_process)} already processed targets"
            )
        ...
        if sequential_mode:
            batch_results = run_batch_predictions_sequential(
                histone_mod_df=histone_mod_df, histone_batch=batch_to_process
            )
        else:
        ...
        for (
            ...
            mi_value,
            self_info_value,
        ) in batch_results:  # Note the additional return value
    ...
    if error_results:
        print(
            f"WARNING: {len(error_results)} targets had errors and were excluded from final results"
        )
    ...
    valid_self_info = {
        k: v
        for k, v in self_info_values.items()
        if not (isinstance(v, str) and v.startswith("Error:"))
    }
    error_self_info = {
        k: v
    ...
    json_file_saver(
        save_dict=sorted_self_info, absolute_file_name=self_info_output_path
    )
